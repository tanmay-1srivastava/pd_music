...
Analysis for Adiba - July 24th Session
"""
...
def create_visual_comparison():
    ...
    with open('Results/individual_sessions/Adiba_July_24_Session_1.json', 'r') as f:
    ...
    # Data from expensive sensors (from your table)
    expensive_data = {
        "Cadence (steps/min)": 111,
        "Gait Speed (m/s)": 0.875,  # Average of L&R
        "Step Duration (s)": 0.541,  # Average of L&R  
        "Stride Length (m)": 0.944,  # Average of L&R
        "Stride Time (s)": 1.08,
        "Double Support (s)": 0.252  # Calculated from %GCT
    }
    ...
    plt.tight_layout()
    plt.savefig('Results/plots/Adiba_Parameter_Comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    comparison_df.to_csv('Results/summary/Adiba_Parameter_Comparison.csv', index=False)
